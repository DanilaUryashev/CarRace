...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        [Space(10)]
        public float nitrousBoostMultiplier = 1.5f;
        public float nitrousDuration = 3.0f;
        public float nitrousCooldown = 5.0f;
        private bool isNitrousReady = true;
        private float currentNitrousTime;
        ...
        private void Update()
        {
            ...
            // Конвертируем м/с в км/ч и берём абсолютное значение
            float currentSpeed = Mathf.Abs(forwardSpeed) * 3.6f;
            speedUI.text = $"{currentSpeed:F0} КМ/Ч";
        ...
        void FixedUpdate()
        {
            // Получаем ввод из GameInput
            accelerationInput = vehicleInput.AccelerationInput;
            steerInput = vehicleInput.SteerInput;
            brakeInput = vehicleInput.BrakeInput;
            ...
            for (int i = 0; i < Wheels.Length; i++)
            {
                ...
                AddSuspensionForce_2(HardPoints[i].position, Wheels[i], MaxSpringDistance,
                                     out wheelHits[i], out wheelIsGrounded, out suspensionForce, i);
            ...
            for (int i = 0; i < Wheels.Length; i++)
            {
                float factor = (i < 2) ? 1 : (brakeInput > 0.1f) ? driftFactor : 1;
                AddLateralFriction_2(HardPoints[i].position, Wheels[i], wheelHits[i],
                                   vehicleIsGrounded, factor, suspensionForce, i);
            }

            NumberOfGroundedWheels = 0;

            // События приземления/взлёта
        ...
        {
            if (vehicleInput.IsNitrousActive && isNitrousReady && accelerationInput > 0)
            {
                if (currentNitrousTime == 0)
                    //VehicleEvents.OnNitrousStart.Invoke();

                currentNitrousTime += Time.fixedDeltaTime;
                accelerationInput *= nitrousBoostMultiplier;

                if (currentNitrousTime >= nitrousDuration)
                {
                    isNitrousReady = false;
                    //VehicleEvents.OnNitrousEnd.Invoke();
                    Invoke(nameof(ResetNitrous), nitrousCooldown);
                }
            }
        }

        private void ResetNitrous()
        {
            isNitrousReady = true;
            currentNitrousTime = 0f;
        }
